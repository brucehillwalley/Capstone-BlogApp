...
export default function DashboardComp() {
    ...
    useEffect(() => {
        const getUsers = async () => {
            try {
                const res = await axiosWithToken.get(`/users?sort[createdAt]=desc&limit=5`);
            // console.log(res.data);
            if (!res.data.error) {
                setUsers(res.data.data);
                setTotalUsers(res.data.details.totalRecords);
        ...
        const getActivities = async () => {
            ...
            if (!res.data.error) {
                ...
                setTotalPosts(res.data.details.totalRecords);
                setLastMonthPosts(res.data.lastMonthActivities);
            }
            } catch (error) {
                console.log(error.message);
            }
      ...
      <div className='flex flex-wrap gap-4 py-3 mx-auto justify-center'>
        ...
        <div className='flex flex-col w-full md:w-auto shadow-md p-2 rounded-md dark:bg-gray-800'>
          <div className='flex justify-between  p-3 text-sm font-semibold'>
            ...
            <Button outline gradientDuoTone='purpleToPink'>
              <Link to={'/dashboard?tab=activities'}>See all</Link>
            </Button>
          </div>
          <Table hoverable>
            <Table.Head>
              ...
              posts.map((post) => (
                <Table.Body key={post._id} className='divide-y'>
                  <Table.Row className='bg-white dark:border-gray-700 dark:bg-gray-800'>
                    ...
                    <Table.Cell className='w-5'>{post.categoryName}</Table.Cell>
                  </Table.Row>
                </Table.Body>
              ))}
          </Table>
